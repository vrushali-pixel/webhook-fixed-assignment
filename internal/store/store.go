...
// IngestEvent atomically applies one webhook delivery: it stores the raw
// event, upserts the call, and folds the call into the durable per-account
// aggregate, all in a single transaction guarded by the unique constraint on
// events.event_id.
//
// If this event_id has already been stored, the INSERT is a no-op (ON
// CONFLICT DO NOTHING) and IngestEvent returns inserted=false without
// touching calls or account_stats -- so a redelivered event, however many
// times it arrives, and even if two copies race each other, is applied
// exactly once. The transaction is what makes this safe under concurrency;
// a separate "does it exist" check beforehand (as the code used to do)
// cannot prevent two concurrent requests from both seeing "not found" and
...
func (s *Store) IngestEvent(ctx context.Context, e Event) (inserted bool, err error) {
	...
	if tag.RowsAffected() == 0 {
		// Someone else already stored this event_id -- nothing further to do.
		return false, nil
	}
